...
// SimFolio - View Performance Utilities
//
...
class ThrottledAction {
    ...
    /// Execute async action with trailing edge - ensures the final call is always executed
    /// This is ideal for slider interactions where you need responsive updates and the final value
    /// - Parameter action: Async action to execute
    @MainActor
    func executeAsyncWithTrailing(_ action: @escaping @Sendable () async -> Void) {
        pendingWorkItem?.cancel()

        let now = Date()
        let timeSinceLastExecution = lastExecutionTime.map { now.timeIntervalSince($0) } ?? minimumInterval
        ...
        if timeSinceLastExecution >= minimumInterval {
            ...
            // Schedule for later to ensure the final call is processed
