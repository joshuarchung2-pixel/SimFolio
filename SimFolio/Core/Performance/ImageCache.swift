...
actor ImageCache {
    ...
    func loadImage(
        ...
        return await withCheckedContinuation { continuation in
            ...
            self.setRequestID(requestID, forKey: cacheKey)
        }
    }
...
struct CachedThumbnailView: View {
    ...
    private var targetSize: CGSize {
        return CGSize(width: size * displayScale, height: size * displayScale)
    }
    ...
    private func loadThumbnail() {
        PhotoLibraryManager.shared.requestEditedThumbnail(
            for: asset,
            size: targetSize
        ) { loadedImage in
