...
// SimFolio - Efficient Photo Library Management
//
...
class EfficientPhotoLibraryManager: NSObject, ObservableObject {
    ...
    private override init() {
        super.init()
        // Don't register as observer until permission is granted
        // setupLibraryChangeObserver() will be called when fetching assets
    }

    /// Whether the change observer has been set up
    ...
    func fetchInitialAssets() async {
        ...
        // Set up change observer now that we have permission
        if !hasSetupObserver {
    ...
    func filteredAssets(
        ...
        let assetsArray: [PHAsset] = assets
        return assetsArray.filter { asset in
            guard let meta = metadata.assetMetadata[asset.localIdentifier] else {
    ...
    func searchAssets(query: String) -> [PHAsset] {
        ...
        return assetsArray.filter { asset in
            ...
            // Search by tooth number if numeric query
            if let toothNum = Int(query), let metaTooth = meta.toothNumber, metaTooth == toothNum {
                return true
            }
