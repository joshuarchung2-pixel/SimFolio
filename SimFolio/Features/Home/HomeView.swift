    ...
    private var recentCapturesSection: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.sm) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: AppTheme.Spacing.sm) {
                    ForEach(Array(library.assets.prefix(20)), id: \.localIdentifier) { asset in
                        RecentThumbnailView(asset: asset)
                    }
                }
                .padding(.horizontal, AppTheme.Spacing.md)
            }
...
struct RecentThumbnailView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            PhotoLibraryManager.shared.requestThumbnail(for: asset, size: CGSize(width: 120, height: 120)) { img in
