...
// SimFolio - Device Orientation Handling
//
...
class OrientationManager: ObservableObject {
    ...
    /// Maps device orientation to EXIF orientation for photo metadata
    /// The mapping accounts for the iPhone camera sensor being mounted in landscape orientation.
    /// When the device is in portrait, the sensor captures a landscape image that needs
    /// to be rotated 90° clockwise for correct display.
    var exifOrientation: CGImagePropertyOrientation {
        switch deviceOrientation {
        case .portrait:
            return .right          // Camera sensor is rotated 90° from portrait
        case .portraitUpsideDown:
            return .left
        case .landscapeLeft:
            return .up             // Natural camera orientation
        case .landscapeRight:
            return .down
        default:
